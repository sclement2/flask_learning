...
original_author="sclement2"
original_name="flask_learning"
original_urlname="flask_learning"
original_description="Awesome flask_learning created by sclement2" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_learning "${name}"
...
then
    ...
    echo "then run 'flask_learning' or 'python -m flask_learning'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_learning' or 'python -m flask_learning' or 'poetry run flask_learning'"
fi
